...
{block name="override_tpl"}
	...
	{* inclusion du js custom *}
	<script type="text/javascript" src="../modules/commandesfournisseuradmin/js/commandes-fournisseurs.js"></script>
					...
					<div class="table-responsive">
						<table class="table" id="orderProducts">
							...
							<tr>
								...
								<th class="text-center"><span class="title_box ">{l s='Qte à la commande'}</span></th>
...
{*					Ajout seb : bouton passer commande fournisseur*}
					<div class="row-margin-bottom row-margin-top order_action">
						...
						>
							...
							{l s='Faire une demande de délai.' d='Module.commandesfournisseursadmin'}
						</button>

						{* modal *}
						<div class="modal fade" data-backdrop="static" tabindex="-1"  aria-labelledby="staticBackdropLabel" aria-hidden="true" role="dialog" id="passer_commande_fournisseur_modal">
			...
			geocoder.geocode({
				...
				if (status === google.maps.GeocoderStatus.OK)
				{
					...
					var delivery_marker = new google.maps.Marker({
						...
						url: 'http://maps.google.com?key=AIzaSyB1S0zFDWWX1Mf-fBDQdoejh-opjweqLnw&q={$addresses.delivery->address1|urlencode},{$addresses.delivery->postcode|urlencode},{$addresses.delivery->city|urlencode}{if isset($addresses.deliveryState->name) && $addresses.delivery->id_state},{$addresses.deliveryState->name|urlencode}{/if},{$addresses.delivery->country|urlencode}'
					});
			...
			geocoder.geocode({
				...
				if (status === google.maps.GeocoderStatus.OK)
				{
					...
					invoice_marker = new google.maps.Marker({
						map: invoice_map,
						position: results[0].geometry.location,
						url: 'http://maps.google.com?key=AIzaSyB1S0zFDWWX1Mf-fBDQdoejh-opjweqLnw&q={$addresses.invoice->address1|urlencode},{$addresses.invoice->postcode|urlencode},{$addresses.invoice->city|urlencode}{if isset($addresses.deliveryState->name) && $addresses.invoice->id_state},{$addresses.deliveryState->name|urlencode}{/if},{$addresses.invoice->country|urlencode}'
					});

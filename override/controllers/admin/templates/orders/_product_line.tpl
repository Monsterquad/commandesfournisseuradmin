...
{if ($product['product_quantity'] > $product['customized_product_quantity'])}
    <tr class="product-line-row">
        {*		ajout colone pour case a cocher pour le module commandesfournisseursadmin*}
        <td>
            {assign var="product_attribute_id" value=$product.product_attribute_id|default:false}
            <input type="checkbox"
                   title="Cocher pour inclure dans la demande de délai."
                   name="mail_fournisseur[{$product.id_product}{if $product_attribute_id}_{$product_attribute_id}{/if}]"
                   data-id_product="{$product.id_product}"
                   data-id_product_attribute="{$product_attribute_id|default:'0'}"
                   data-reference="{$product.product_reference|default:''|escape}"
                   data-order_ref="{$order->reference|escape}"
            ></td>
        {*		fin ajout*}

        <td>{if isset($product.image) && $product.image->id}{$product.image_tag}{/if}</td>

        {*ajout ici pour le module - quantité avant commande*}
        <td>{if isset($product.product_reference) && $product.product_reference}{$product.product_reference}{/if}</td>
        {*fin ajout*}
        ...
        <td>
            <a href="{$link->getAdminLink('AdminProducts', true, ['id_product' => $product['product_id']|intval, 'updateproduct' => '1'])|escape:'html':'UTF-8'}">
                ...
                {if $product.product_reference}{l s='Reference number:' d='Admin.Orderscustomers.Feature'} {$product.product_reference}
                    <br/>
                {/if}
            ...
            {if isset($product.pack_items) && $product.pack_items|@count > 0}
                <br>
                <button name="package" class="btn btn-default" type="button"
                        onclick="TogglePackage('{$product['id_order_detail']}'); return false;"
                        value="{$product['id_order_detail']}">{l s='Package content' d='Admin.Orderscustomers.Feature'}</button>
            {/if}
        ...
        <td>
            ...
            {if $can_edit}
                <div class="product_price_edit" style="display:none;">
                    <input type="hidden" name="product_id_order_detail" class="edit_product_id_order_detail"
                           value="{$product['id_order_detail']}"/>
                    <div class="form-group">
                        <div class="fixed-width-xl">
                            <div class="input-group">
                                {if $currencySymbolBeforeAmount}
                                    <div class="input-group-addon">{$currency->sign} {l s='tax excl.' d='Admin.Global'}</div>{/if}
                                <input type="text" name="product_price_tax_excl"
                                       class="edit_product_price_tax_excl edit_product_price"
                                       value="{Tools::ps_round($product['unit_price_tax_excl'], 2)}"/>
                                {if !$currencySymbolBeforeAmount}
                                    <div class="input-group-addon">{$currency->sign} {l s='tax excl.' d='Admin.Global'}</div>{/if}
                            </div>
                        </div>
                        ...
                        <div class="fixed-width-xl">
                            <div class="input-group">
                                {if $currencySymbolBeforeAmount}
                                    <div class="input-group-addon">{$currency->sign} {l s='tax incl.' d='Admin.Global'}</div>{/if}
                                <input type="text" name="product_price_tax_incl"
                                       class="edit_product_price_tax_incl edit_product_price"
                                       value="{Tools::ps_round($product['unit_price_tax_incl'], 2)}"/>
                                {if !$currencySymbolBeforeAmount}
                                    <div class="input-group-addon">{$currency->sign} {l s='tax incl.' d='Admin.Global'}</div>{/if}
                            </div>
                        </div>
        ...
        <td class="productQuantity text-center">
			...
			<input type="text" name="product_quantity" class="edit_product_quantity"
                   value="{$product['product_quantity']|htmlentities}"/>
		</span>
            {/if}
        </td>
        ...
        <td class="productQuantity text-center">
            <span class="product_quantity_show{if (int)$product['quantity_in_stock_at_order_time'] < 1000} badge badge-danger{/if}">{$product['quantity_in_stock_at_order_time']}</span>
        </td>
        {*fin ajout*}
        ...
        {if $display_warehouse}
            <td>
                ...
                {if $product.warehouse_location}
                    <br>
                    {l s='Location' d='Admin.Orderscustomers.Feature'}:
                    <strong>{$product.warehouse_location|escape:'html':'UTF-8'}</strong>
                {/if}
            </td>
        {/if}
        {if ($order->hasBeenPaid())}
            <td class="productQuantity text-center">
                ...
                <input type="hidden"
                       value="{(Tools::ps_round($product_price, 2) * ($product['product_quantity'] - $product['customizationQuantityTotal']))}"
                ...
                {if count($product['refund_history'])}
					...
					{foreach $product['refund_history'] as $refund}
                        {l s='%refund_date% - %refund_amount%' sprintf=['%refund_date%' => {dateFormat date=$refund.date_add}, '%refund_amount%' => {displayPrice price=$refund.amount_tax_incl}] d='Admin.Orderscustomers.Feature'}
                        <br/>
                    {/foreach}
					</span>
				</span>
            ...
            <td class="productQuantity text-center">
                ...
                {if count($product['return_history'])}
					...
					{foreach $product['return_history'] as $return}
                        {l s='%return_date% - %return_quantity% - %return_state%' sprintf=['%return_date%' =>{dateFormat date=$return.date_add}, '%return_quantity%' => $return.product_quantity, '3return_state%' => $return.state] d='Admin.Orderscustomers.Feature'}
                        <br/>
                    {/foreach}
					</span>
				</span>
        ...
        {/if}
        {if $stock_location_is_available}
            <td class="productQuantity location text-center">{$product['location']}</td>{/if}
        {if $stock_management}
            <td class="productQuantity product_stock text-center">{$product['current_stock']}</td>{/if}
        ...
        <td class="cancelCheck standard_refund_fields current-edit" style="display:none">
            <input type="hidden" name="totalQtyReturn" id="totalQtyReturn"
                   value="{$product['product_quantity_return']}"/>
            ...
            {if ((!$order->hasBeenDelivered() OR Configuration::get('PS_ORDER_RETURN')) AND (int)($product['product_quantity_return']) < (int)($product['product_quantity']))}
                <input type="checkbox" name="id_order_detail[{$product['id_order_detail']}]"
                       id="id_order_detail[{$product['id_order_detail']}]" value="{$product['id_order_detail']}"
                       onchange="setCancelQuantity(this, {$product['id_order_detail']}, {$product['product_quantity'] - $product['customizationQuantityTotal'] - $product['product_quantity_return'] - $product['product_quantity_refunded']})"
                       {if ($product['product_quantity_return'] + $product['product_quantity_refunded'] >= $product['product_quantity'])}disabled="disabled" {/if}/>
            {else}
                --
            {/if}
        </td>
        <td class="cancelQuantity standard_refund_fields current-edit" style="display:none">
            ...
            {elseif (!$order->hasBeenDelivered() OR Configuration::get('PS_ORDER_RETURN'))}
                <input type="text" id="cancelQuantity_{$product['id_order_detail']}"
                       name="cancelQuantity[{$product['id_order_detail']}]"
        ...
        <td class="partial_refund_fields current-edit" colspan="2"
            style="display:none; width: 250px; min-width: 250px;">
            {if $product['quantity_refundable'] > 0}
                ...
                <div class="form-group">
                    <div class="{if $product['amount_refundable'] > 0}col-lg-4{else}col-lg-12{/if}">
                        ...
                        <div class="input-group">
                            <input onchange="checkPartialRefundProductQuantity(this)" type="text"
                                   name="partialRefundProductQuantity[{{$product['id_order_detail']}}]" value="0"/>
                    ...
                    <div class="{if $product['quantity_refundable'] > 0}col-lg-8{else}col-lg-12{/if}">
                        ...
                        <div class="input-group">
                            {if $currencySymbolBeforeAmount}
                                <div class="input-group-addon">{$currency->sign}</div>{/if}
                            <input onchange="checkPartialRefundProductAmount(this)" type="text"
                                   name="partialRefundProduct[{$product['id_order_detail']}]"/>
                            {if !$currencySymbolBeforeAmount}
                                <div class="input-group-addon">{$currency->sign}</div>{/if}
                        </div>
                        <p class="help-block"><i
                                    class="icon-warning-sign"></i> {l s='(Max %amount_refundable% %tax_method%)' sprintf=[ '%amount_refundable%' => Tools::displayPrice(Tools::ps_round($amount_refundable, 2), $currency->id), '%tax_method%' => $smarty.capture.TaxMethod] d='Admin.Orderscustomers.Help'}
                        </p>
                    </div>
                </div>
            {/if}
        </td>
        {* date de commande fournisseur - ajout seb *}
        <td>
            {if $product['date_commande_fournisseur']}
                {$product['date_commande_fournisseur']|date_format:'%d/%m/%Y'}
            {else}
                non demandé
            {/if}
        </td>
                    ...
                    <select name="product_invoice" class="edit_product_invoice">
                        {foreach from=$invoices_collection item=invoice}
                            <option value="{$invoice->id}"
                                    {if $invoice->id == $product['id_order_invoice']}selected="selected"{/if}>
        ...
        <tr>
            <td colspan="8" style="width:100%">
                <table style="width: 100%; display:none;" class="table" id="pack_items_{$product['id_order_detail']}">
                    ...
                    {if $stock_management}
                        <th><span class="title_box ">{l s='Available quantity' d='Admin.Orderscustomers.Feature'}</span>
                        </th>{/if}
                    <th>&nbsp;</th>
                    </thead>
                    <tbody>
                            ...
                            <tr class="product-line-row" {if isset($pack_item.image) && $pack_item.image->id && isset($pack_item.image_size)} height="{$pack_item['image_size'][1] + 7}"{/if}>
                                ...
                                <td>
                                    <a href="{$link->getAdminLink('AdminProducts', true, ['id_product' => $pack_item.id_product, 'updateproduct' => '1'])|escape:'html':'UTF-8'}">
                                        ...
                                        {if $pack_item.reference}{l s='Ref:' d='Admin.Orderscustomers.Feature'} {$pack_item.reference}
                                            <br/>
                                        {/if}
                                ...
                                {if $stock_management}
                                    <td class="productQuantity product_stock">{$pack_item.current_stock}</td>{/if}
                                <td>&nbsp;</td>
                            </tr>
                        {/if}
                    {/foreach}
